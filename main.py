...
if __name__ == "__main__":
    ...
    try:
        ...
        if not opt.finetune_from == "":
            ...
            pdb.set_trace()
            # manually delete denoising unet weights for ablation training from scratch:
            for weight_key in tqdm(old_state):
                if weight_key.startswith("model.diffusion_model"):
                    unet_weight = old_state[weight_key]
                    print(f"Manual init: {weight_key}")
                    unet_weight = unet_weight.zero_()
                    old_state[weight_key] = torch.nn.parameter.Parameter(unet_weight)
